...
@section('section')
    <nav id="orders-table-tab" class="orders-table-tab app-nav-tabs nav shadow-sm flex-column flex-sm-row mb-4">
        <a class="flex-sm-fill text-sm-center nav-link {{ Request::has('month') || Request::has('year') ? ' ' : 'active' }}"
            ...
            aria-selected="{{ Request::has('month') || Request::has('year') ? 'false' : 'true' }}">All</a>
        <a class="flex-sm-fill text-sm-center nav-link" id="orders-paid-tab" data-bs-toggle="tab"
            href="{{ Request::has('month') || Request::has('year') ? ' ' : '#orders-paid' }}" role="tab"
            aria-controls="orders-paid" aria-selected="false">Today</a>
        <a class="flex-sm-fill text-sm-center nav-link" id="orders-pending-tab" data-bs-toggle="tab"
            href="{{ Request::has('month') || Request::has('year') ? ' ' : '#orders-pending' }}" role="tab"
            aria-controls="orders-pending" aria-selected="false">This Month</a>
    </nav>
                            ...
                            <tbody>
                                @foreach ($all as $item)
                                    ...
                                    <tr>
                                        ...
                                        <td class="cell text-center">{{ $item->meja->nama }}</td>
                                        <td class="cell"><span>{{ $date->toFormattedDateString() }}</span><span
                                                class="note">{{ $time[4] . ' ' . $time[5] }}</span></td>
                                        <td class="cell"><span
                                                class="badge {{ $item->status ? 'bg-success' : 'bg-danger' }}">{{ $item->status ? 'Dibayar' : 'Belum dibayar' }}</span>
                                        </td>
                                        <td class="cell">Rp. {{ number_format($item->total_transaksi, 0, ',', '.') }}
                                        </td>
                                        <td class="cell"><a class="btn-sm app-btn-secondary"
                                                href="{{ route('transaksi.show', $item->id) }}">View</a></td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
            ...
            elem.addEventListener('click', function(event) {
                ...
                if (queryString) {
                    ...
                    if (id == 'orders-paid-tab') {
                        print.href = '/laporan?data=today';
                    } else if (id == 'orders-pending-tab') {
                        print.href = '/laporan?data=thisMonth';
                    } else {
                        print.href = '/laporan?data=all';
                    }
                    return false;
                }
